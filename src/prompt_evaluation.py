...
def generate_prompt_evaluation_report(evaluation_results: list[dict[str, str | int | dict]]) -> str:
    """
    Generates an HTML report summarizing the prompt evaluation results.

    Args:
        evaluation_results (list[dict[str, str | int | dict]]): A list of dictionaries,
            each containing the output, test case, score, and reasoning for a
            single test case evaluation.

    Returns:
        str: An HTML string representing the evaluation report.
    """
...
class PromptEvaluator:
    def __init__(self, max_concurrent_tasks: int = 3):
        """
        Initializes the PromptEvaluator with a specified maximum number of concurrent tasks.
        """
        self.max_concurrent_tasks = max_concurrent_tasks

    def render(self, template_string: str, variables: dict) -> str:
        """
        Renders a template string by replacing placeholders with corresponding values from the variables dictionary.

        Placeholders in the template string should be in the format {placeholder_name}.

        Args:
            template_string (str): The string containing placeholders to be replaced.
            variables (dict): A dictionary where keys are placeholder names and values are the values to replace

        Returns:
            str: The rendered string with placeholders replaced.
        """
    ...
    def generate_test_case(
        ...
        The prompt_inputs_spec is a dictionary where keys are input names and values are descriptions of those inputs.
        This will be used to guide the generation of the test case and ensure it includes the necessary inputs.
        ...
        The output will be a dictionary with the following structure:
        {
            ...
            "solution_criteria": [
                // List of criteria for evaluating the solution
            ]
        }

        Args:
            task_description (str): A description of the task that the prompt is designed to accomplish.
            idea (str): A specific scenario or example that tests a particular aspect of the task.
            prompt_inputs_spec (dict): A dictionary specifying the expected inputs for the prompt, where keys
                are input names and values are descriptions of those inputs.

        Returns:
            dict: A dictionary representing the test case, including the prompt inputs and solution criteria.
        """
    ...
    def generate_dataset(
        ...
        Args:
            ...
            output_file (str): The file path where the generated dataset will be saved.
    ...
    def grade_output(
        self, test_case: dict, output: str, extra_criteria: str | None
        ...
        Args:
            test_case (dict): The test case to grade.
            output (str): The output generated.
            extra_criteria (str | None): Additional criteria for evaluation.

        Returns:
            dict[str, str | int | dict]: The evaluation results.
        """
    ...
    def run_test_case(
        ...
        Args:
            ...
            run_prompt_function (Callable): The function to run the prompt.
            extra_criteria (str | None): Additional criteria for evaluation.
    ...
    def run_evaluation(
        ...
        Args:
            run_prompt_function (Callable): The function to run the prompt.
            dataset_file (str): The file path of the dataset to evaluate.
            extra_criteria (str | None): Additional criteria for evaluation.
            json_output_file (str): The file path where the evaluation results will be saved in JSON
            html_output_file (str): The file path where the evaluation report will be saved in HTML.

        Returns:
            list[dict[str, str | int | dict]]: A list of evaluation results for each test case.
        """
